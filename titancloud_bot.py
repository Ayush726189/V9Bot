...
def env_int(name, default, minimum=1, maximum=None):
    """Read a bounded integer setting without making a bad .env crash startup."""
    ...
    if value < minimum or (maximum is not None and value > maximum):
        logger.warning("Out-of-range %s=%r; using %s", name, raw_value, default)
        return default
    return value
...
MAX_VPS_PER_USER = env_int('MAX_VPS_PER_USER', 1)
VPS_MEMORY_GB = env_int('DEFAULT_MEMORY_GB', 8)
DEFAULT_CPU_CORES = env_int('DEFAULT_CPU_CORES', 4, maximum=32)
DEFAULT_DISK_GB = env_int('DEFAULT_DISK_GB', 60, minimum=10, maximum=1000)
MAX_CUSTOM_MEMORY_GB = env_int(
    'MAX_CUSTOM_MEMORY_GB', max(256, VPS_MEMORY_GB), minimum=VPS_MEMORY_GB
)
MAX_CUSTOM_CPU_CORES = env_int(
    'MAX_CUSTOM_CPU_CORES', max(64, DEFAULT_CPU_CORES), minimum=DEFAULT_CPU_CORES
)
MAX_CUSTOM_DISK_GB = env_int(
    'MAX_CUSTOM_DISK_GB', max(2000, DEFAULT_DISK_GB), minimum=DEFAULT_DISK_GB
)
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = env_int('MAX_CONTAINERS', 250)
DB_FILE = 'titancloud.db'
BACKUP_FILE = 'titancloud_backup.json'
...
EMOJI_DECLINE = "<:red_dot:1536292733528838195>"
...
VPS_COLUMN_SET = frozenset(VPS_COLUMNS)
...
def premium_embed(title, description=None, color=BRAND_COLOR):
    ...
    embed.set_author(name="TITANCLOUD  /  PRIVATE COMPUTE")
    embed.set_footer(text="Secure infrastructure • Managed from Discord")
    embed.timestamp = discord.utils.utcnow()
    return embed


def add_instance_spec_fields(embed, vps):
    """Add the compact three-column resource row used by premium embeds."""
...
def build_control_panel_embed(vps_id, vps, container_status=None):
    """Build the canonical clean VPS control-panel embed."""
    service_status = str(vps.get('status', 'unknown')).upper()
    docker_status = str(container_status or 'unknown').upper()
    color = SUCCESS_COLOR if service_status == 'RUNNING' and docker_status == 'RUNNING' else WARNING_COLOR
    if service_status == 'SUSPENDED' or docker_status == 'NOT FOUND':
        color = ERROR_COLOR

    status_emoji = EMOJI_START if service_status == 'RUNNING' and docker_status == 'RUNNING' else EMOJI_STOP
    if service_status == 'SUSPENDED':
        status_emoji = EMOJI_RESTART

    embed = premium_embed(
        f"{EMOJI_MANAGE}  CONTROL PANEL  /  {vps_id}",
        "Your private compute instance. Use the controls below; sensitive access details are delivered only by DM.",
        color
    )
    embed.add_field(
        name=f"{status_emoji}  INSTANCE STATUS",
        value=f"Service `{service_status}`  •  Docker `{docker_status}`",
        inline=False
    )
    add_instance_spec_fields(embed, vps)
    ...
    embed.add_field(
        name=f"{EMOJI_MANAGE}  SECURE ACCESS",
        value="Select **Fresh SSH** to revoke the previous tmate session and receive a new private command by DM.",
        inline=False
    )
    return embed


def build_ssh_access_embed(vps_id, ssh_session):
    """Build a focused DM containing only the private SSH session."""
    embed = premium_embed(
        f"{EMOJI_VERIFY}  SSH ACCESS  /  {vps_id}",
        "A new private session is active. Every earlier tmate session for this VPS has been closed.",
        SUCCESS_COLOR
    )
    embed.add_field(name=f"{EMOJI_START}  STATUS", value="`READY`", inline=True)
    embed.add_field(name=f"{EMOJI_MANAGE}  PRIVATE SSH COMMAND", value=f"```bash\n{ssh_session}\n```", inline=False)
    embed.add_field(
        name=f"{EMOJI_RESTART}  IMPORTANT",
        value="Copy the command, paste it into a terminal, and press Enter. Do not share this message or command.",
        inline=False
    )
    return embed
...
class Database:
    ...
    def _initialize_settings(self):
        ...
        # Keep persisted limits synchronized with the current .env values.
        self.cursor.execute(
    ...
    def add_vps(self, vps_data):
        invalid_columns = set(vps_data) - VPS_COLUMN_SET
        if invalid_columns:
            raise ValueError(f"Invalid VPS columns: {sorted(invalid_columns)}")
        columns = ', '.join(vps_data.keys())
        placeholders = ', '.join('?' for _ in vps_data)
        self.cursor.execute(  # noqa: S608 - column names are allowlisted above
            f'INSERT INTO vps_instances ({columns}) VALUES ({placeholders})',  # noqa: S608
            tuple(vps_data.values())
        )
        self.conn.commit()
    ...
    def update_vps(self, token, updates):
        invalid_columns = set(updates) - (VPS_COLUMN_SET - {'token'})
        if invalid_columns:
            raise ValueError(f"Invalid VPS columns: {sorted(invalid_columns)}")
        if not updates:
            return False
        ...
        self.cursor.execute(  # noqa: S608 - column names are allowlisted above
            f'UPDATE vps_instances SET {set_clause} WHERE token = ?',  # noqa: S608
            values
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
    ...
    def backup_data(self):
        """Back up all data to a non-executable JSON file."""
        data = {
        ...
        temp_backup = f"{BACKUP_FILE}.tmp"
        with open(temp_backup, 'w', encoding='utf-8') as backup_stream:
            json.dump(data, backup_stream, indent=2, ensure_ascii=False)
        os.replace(temp_backup, BACKUP_FILE)

        return True

    def restore_data(self):
        """Restore validated data from the JSON backup file atomically."""
        if not os.path.exists(BACKUP_FILE):
            return False

        try:
            with open(BACKUP_FILE, encoding='utf-8') as backup_stream:
                data = json.load(backup_stream)

            required_sections = {
                'vps_instances', 'usage_stats', 'system_settings',
                'banned_users', 'admin_users'
            }
            if not isinstance(data, dict) or not required_sections.issubset(data):
                raise ValueError("Backup file is missing required sections")
            if not isinstance(data['vps_instances'], dict):
                raise ValueError("Backup VPS data must be an object")
            for vps in data['vps_instances'].values():
                if not isinstance(vps, dict) or set(vps) - VPS_COLUMN_SET:
                    raise ValueError("Backup contains invalid VPS columns")
            ...
            for table in (
                ...
                self.cursor.execute(f'DELETE FROM {table}')  # noqa: S608 - fixed table allowlist

            # Restore VPS instances
            for token, vps in data['vps_instances'].items():
                if vps.get('token') != token:
                    raise ValueError("Backup VPS token key does not match its record")
                columns = [column for column in VPS_COLUMNS if column in vps]
                placeholders = ', '.join('?' for _ in columns)
                column_sql = ', '.join(columns)
                self.cursor.execute(  # noqa: S608 - columns come from VPS_COLUMNS
                    f'INSERT INTO vps_instances ({column_sql}) VALUES ({placeholders})',  # noqa: S608
                    tuple(vps[column] for column in columns)
                )
...
class TitanCloudBot(commands.Bot):
    def __init__(self, *args, **kwargs):
        ...
        self.capacity_guard = asyncio.Lock()
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            logger.exception("Unexpected error reconnecting saved containers: %s", exc)
        try:
            await self.restore_persistent_views()
        except Exception as exc:
            logger.exception("Could not restore persistent control panels: %s", exc)
    ...
    async def restore_persistent_views(self):
        """Restore every saved VPS control panel after a bot restart."""
        restored = 0
        for vps in self.db.get_all_vps().values():
            container_id = vps.get('container_id')
            vps_id = vps.get('vps_id')
            if not container_id or not vps_id:
                continue
            view = VPSManagementView(vps_id, container_id)
            self.add_view(view)
            self.my_persistent_views[vps_id] = view
            restored += 1
        logger.info("Restored %s persistent VPS control panels", restored)
                        ...
                        for pattern in MINER_PATTERNS:
                            if pattern in output:
                                ...
                                try:
                                    ...
                                    logger.warning("Could not notify suspended VPS owner: %s", exc)
                                break
                    except Exception as e:
...
def generate_vps_id():
    """Generate a hard-to-guess VPS ID."""
    alphabet = string.ascii_uppercase + string.digits
    return ''.join(secrets.choice(alphabet) for _ in range(10))

def make_linux_username(display_name, user_id):
    """Convert any Discord display name into a safe Linux account name."""
    username = re.sub(r'[^a-z0-9_-]+', '_', display_name.lower()).strip('_-')
    if not username or not re.match(r'^[a-z_]', username):
...
async def defer_context(ctx, ephemeral=False):
    ...
    if interaction and not interaction.response.is_done():
        try:
            ...
            logger.warning("Discord interaction could not be deferred: %s", exc)
...
async def safe_status_update(target, content):
    """Best-effort progress reporting that can never abort provisioning."""
    if target is None:
        return None
    try:
        if isinstance(target, discord.Interaction):
            if target.response.is_done():
                return await target.followup.send(content, ephemeral=True)
        ...
        logger.warning("Discord status update skipped: %s", exc)
    except Exception as exc:
        logger.warning("Status update skipped: %s", exc)
    return None


async def safe_interaction_send(interaction, content=None, **kwargs):
    """Respond to a component interaction without leaking webhook failures."""
    try:
        if interaction.response.is_done():
            return await interaction.followup.send(content, **kwargs)
        return await interaction.response.send_message(content, **kwargs)
    except (discord.NotFound, discord.Forbidden, discord.HTTPException) as exc:
        logger.warning("Discord interaction response skipped: %s", exc)
    return None
...
async def run_docker_command(container_id, command, timeout=300):
...
async def generate_fresh_ssh_session(container_id):
    """Atomically revoke old tmate sessions and return one new SSH command."""
    lock = SSH_SESSION_LOCKS.setdefault(container_id, asyncio.Lock())
    async with lock:
        last_error = "tmate did not return a valid SSH session"
        for attempt in range(2):
            socket_name = f"/tmp/titancloud-{generate_token()[:12]}.sock"  # noqa: S108
            command = (
                "pkill -TERM -x tmate >/dev/null 2>&1 || true; "
                "for i in 1 2 3 4 5; do "
                "pgrep -x tmate >/dev/null 2>&1 || break; sleep 0.1; done; "
                "pkill -KILL -x tmate >/dev/null 2>&1 || true; "
            ...
            if success:
                ...
                if session.startswith('ssh '):
                    return session
                last_error = "tmate returned an invalid SSH session"
            else:
                last_error = output
...
async def wait_for_apt_lock(container_id, status_msg):
    ...
    for attempt in range(max_attempts):
        try:
            ...
            await safe_status_update(
                status_msg,
                f"<a:48084loadingcircle:1536290034930548786> Waiting for package manager to be ready... (Attempt {attempt + 1}/{max_attempts})"
            )
            await asyncio.sleep(5)
        except Exception as e:
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        await safe_status_update(status_msg, f"{EMOJI_MANAGE} Checking container status...")

        container = await docker_call(bot.docker_client.containers.get, container_id)
        if container.status != "running":
            await safe_status_update(status_msg, f"{EMOJI_START} Starting container...")
            await docker_call(container.start)
            await asyncio.sleep(5)

        # Install tmate and other required packages
        if not use_custom_image:
            await safe_status_update(status_msg, f"{EMOJI_LOADING} Installing required packages...")
        ...
        # Create the internal Linux account. Remote access itself is tmate-only;
        # no OpenSSH daemon or password-login port is configured in-container.
        await safe_status_update(status_msg, f"{EMOJI_MANAGE} Preparing the private console...")

        # Create a locked internal account for local container organization.
        if not use_custom_image:
            user_setup_commands = [
        ...
        await safe_status_update(status_msg, f"{EMOJI_MANAGE} Applying TitanCloud customization...")

        # Create welcome message file
        welcome_cmd = (
            f"echo '{WELCOME_MESSAGE}' > /etc/motd && "
            f"echo 'echo \"{WELCOME_MESSAGE}\"' >> /home/{username}/.bashrc && "
            f"echo 'echo \"{WELCOME_MESSAGE}\"' >> /root/.bashrc"
        )
        ...
        if not success:
            logger.warning(f"Could not set in-container hostname: {output}")
        ...
        await safe_status_update(status_msg, f"{EMOJI_LOADING} Verifying resource limits...")
        ...
        # Container-local housekeeping. Network access is controlled by Docker
        # port publishing and the host firewall, never from inside the container.
        ...
        for cmd in housekeeping_commands:
            success, output = await run_docker_command(container_id, ["bash", "-c", cmd])
            if not success:
                logger.warning(f"Container housekeeping command failed: {cmd} - {output}")

        await safe_status_update(status_msg, f"{EMOJI_VERIFY} TitanCloud VPS setup completed successfully!")

        return True, None, vps_id
    except Exception as e:
        error_msg = f"Setup failed: {str(e)}"
        logger.error(error_msg)
        await safe_status_update(status_msg, f"{EMOJI_DECLINE} {error_msg}")
...
async def on_ready():
    ...
    logger.info(
        "Configured VPS plan: %s GB RAM, %s vCPU, %s GB displayed storage, %s VPS/user",
        VPS_MEMORY_GB,
        DEFAULT_CPU_CORES,
        DEFAULT_DISK_GB,
        MAX_VPS_PER_USER
...
async def show_commands(ctx):
    ...
    try:
        ...
        embed.add_field(
            name="YOUR PLAN",
            value=(
                f"`{VPS_MEMORY_GB} GB RAM`  /  `{DEFAULT_CPU_CORES} vCPU`  /  "
                f"`{DEFAULT_DISK_GB} GB storage`  /  `{MAX_VPS_PER_USER} VPS per regular user`"
            ),
            inline=False
        )
        embed.add_field(
        ...
        if has_admin_role(ctx):
            embed.add_field(
                name="OWNER TOOLKIT",
                value=(
                    "`/create_vps target:@member ram:16 disk:120 core:8`  •  custom high-spec VPS\n"
                    "`/vps_list`  •  `/delete_vps`  •  `/admin_stats`\n"
...
def custom_plan_error(ram, disk, core):
    """Return a user-facing validation error for an owner custom plan."""
    if ram < VPS_MEMORY_GB or core < DEFAULT_CPU_CORES or disk < DEFAULT_DISK_GB:
        return (
            "Custom plans cannot be smaller than the normal plan: "
            f"{VPS_MEMORY_GB} GB RAM, {DEFAULT_CPU_CORES} cores, "
            f"and {DEFAULT_DISK_GB} GB disk."
        )
    if ram > MAX_CUSTOM_MEMORY_GB:
        return f"RAM cannot exceed {MAX_CUSTOM_MEMORY_GB} GB."
    if core > MAX_CUSTOM_CPU_CORES:
        return f"CPU cores cannot exceed {MAX_CUSTOM_CPU_CORES}."
    if disk > MAX_CUSTOM_DISK_GB:
        return f"Disk cannot exceed {MAX_CUSTOM_DISK_GB} GB."
    return None
...
async def provision_vps(
    ...
    *,
    enforce_user_quota,
    private_progress=False
):
    """Provision one container through the shared, serialized deployment path."""
    os_image = DEFAULT_OS_IMAGE
    # Nested VPS hosts commonly reject Docker overlay mounts during `docker
    # build`. Deploy the base Ubuntu image directly and configure it in-place.
    use_custom_image = False

    if bot.db.is_user_banned(owner.id):
        await safe_ctx_send(ctx, f"{SYMBOL['error']} Your VPS access is suspended.", ephemeral=True)
        return

    if not bot.docker_client:
        await safe_ctx_send(ctx, f"{SYMBOL['error']} Provisioning is temporarily unavailable. Contact an owner.", ephemeral=True)
        return
    ...
    try:
        # Reserve a global slot atomically so simultaneous owner deployments
        # cannot race past MAX_CONTAINERS.
        async with bot.capacity_guard:
            ...
            if len(containers) + bot.reserved_capacity >= max_containers:
                await safe_ctx_send(ctx, f"{SYMBOL['error']} Capacity is full. Contact an owner.", ephemeral=True)
                return
            bot.reserved_capacity += 1
        ...
        if enforce_user_quota and bot.db.get_user_vps_count(owner.id) >= MAX_VPS_PER_USER:
            await safe_ctx_send(ctx, f"{SYMBOL['info']} You already have a container. Use `/list` or `/manage`.", ephemeral=True)
            return

        status_msg = await safe_ctx_send(
            ctx,
            f"{SYMBOL['progress']} Reserving a {memory} GB container for {owner.mention}...",
            ephemeral=private_progress
        )

        memory_bytes = memory * 1024 * 1024 * 1024
        vps_id = generate_vps_id()
        username = make_linux_username(owner.name, owner.id)
        token = generate_token()

        await safe_status_update(status_msg, f"{SYMBOL['progress']} Preparing the Ubuntu runtime...")
        run_options = {
            "detach": True,
            "privileged": True,
            ...
            "volumes": {f'titancloud-{vps_id}': {'bind': '/data', 'mode': 'rw'}},
            "restart_policy": {"Name": "always"}
        }
        try:
            container = await docker_call(bot.docker_client.containers.run, os_image, **run_options)
        except docker.errors.ImageNotFound:
            await safe_status_update(status_msg, f"{SYMBOL['progress']} Downloading `{os_image}`...")
            await docker_call(bot.docker_client.images.pull, os_image)
            container = await docker_call(bot.docker_client.containers.run, os_image, **run_options)

        await safe_status_update(status_msg, f"{SYMBOL['progress']} Applying the TitanCloud environment...")
        ...
        if not setup_success:
            raise RuntimeError("Failed to set up the container")

        await safe_status_update(status_msg, f"{SYMBOL['progress']} Securing the remote access session...")
        ...
        try:
            embed = premium_embed(
                f"{EMOJI_VERIFY}  DEPLOYMENT COMPLETE",
                "Your private compute instance is online and ready for secure access.",
                SUCCESS_COLOR
            )
            embed.add_field(name="INSTANCE", value=f"`{vps_id}`", inline=True)
            embed.add_field(name="STATUS", value="`READY`", inline=True)
            embed.add_field(name="SYSTEM", value=f"`{os_image}`", inline=True)
            add_instance_spec_fields(embed, vps_data)
            embed.add_field(
                name=f"{EMOJI_MANAGE}  PRIVATE SSH COMMAND",
                value=f"```bash\n{ssh_session_line}\n```",
                inline=False
            )
            embed.add_field(
                name=f"{EMOJI_RESTART}  CONNECT & PROTECT",
                value="Paste the command into a terminal. Keep this DM private. Use **Fresh SSH** in `/manage` to revoke it and receive a replacement. No username or password is required.",
                inline=False
            )

            await owner.send(embed=embed)
            await safe_status_update(status_msg, f"{SYMBOL['success']} Deployment complete, {owner.mention}. Access details were sent by DM.")
        except discord.Forbidden:
            await safe_status_update(status_msg, f"{SYMBOL['info']} Container created, but your DMs are closed. Enable server DMs and contact an owner.")
        except discord.HTTPException as exc:
        ...
        if not provisioned and container is not None:
            try:
                await docker_call(container.stop)
...
@bot.hybrid_command(name='deploy', description=f'Deploy a premium {VPS_MEMORY_GB} GB VPS')
async def deploy_command(ctx):
    """Deploy the environment-configured normal VPS plan."""
    await defer_context(ctx)

    if not ctx.guild:
        await safe_ctx_send(ctx, f"{SYMBOL['error']} This command can only be used in a server.", ephemeral=True)
        return

    owner_request = is_owner(ctx)
    if not owner_request and not has_user_role(ctx):
        await safe_ctx_send(ctx, f"{SYMBOL['error']} You need the <@&{USER_ROLE_ID}> role to deploy a VPS.", ephemeral=True)
        return

    owner = ctx.author
    if owner.bot:
        await safe_ctx_send(ctx, f"{SYMBOL['error']} A VPS cannot be assigned to a bot account.", ephemeral=True)
        return
    ...
    await provision_vps(
        ...
        DEFAULT_CPU_CORES,
        DEFAULT_DISK_GB,
        enforce_user_quota=not owner_request
    )


@bot.hybrid_command(name='create_vps', description='Owner only: create a custom high-spec VPS')
@app_commands.describe(
    target="Member who will own and receive the VPS",
    ram="RAM allocation in GB",
    disk="Displayed disk allocation in GB",
    core="CPU core allocation"
)
async def create_vps_command(ctx, target: discord.Member, ram: int, disk: int, core: int):
    """Create a larger custom-plan VPS for a selected member."""
    ...
    if not ctx.guild:
        ...
        return
    if not is_owner(ctx):
        await safe_ctx_send(ctx, f"{SYMBOL['error']} This command is restricted to TitanCloud owners.", ephemeral=True)
        return
    if target.bot:
        await safe_ctx_send(ctx, f"{SYMBOL['error']} A VPS cannot be assigned to a bot account.", ephemeral=True)
        return

    validation_error = custom_plan_error(ram, disk, core)
    if validation_error:
        await safe_ctx_send(ctx, f"{SYMBOL['error']} {validation_error}", ephemeral=True)
        return
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = premium_embed(
            title=f"{EMOJI_MANAGE}  YOUR VPS INSTANCES",
            description="Select an instance with `/manage <vps_id>` to control it or request private SSH.",
            color=BRAND_COLOR
        )
        ...
        for vps in user_vps:
            try:
                ...
                if vps["container_id"]:
                    await docker_call(bot.docker_client.containers.get, vps["container_id"])
            ...
            embed.add_field(
                name=f"{EMOJI_START if status == 'Running' else EMOJI_STOP}  {vps['vps_id']}",
                value=(
                    f"Status `{status.upper()}`  •  System `{vps.get('os_image', DEFAULT_OS_IMAGE)}`\n"
                    f"`{vps.get('memory', 'Unknown')} GB RAM`  •  `{vps.get('cpu', 'Unknown')} vCPU`  •  "
                    f"`{vps.get('disk', 'Unknown')} GB storage`\n"
                    f"Restarts `{vps.get('restart_count', 0)}`"
                ),
                inline=False
            )
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = premium_embed(title=f"{EMOJI_MANAGE}  ALL VPS INSTANCES", color=BRAND_COLOR)
        valid_vps_count = 0
        ...
        for token, vps in all_vps.items():
            ...
            try:
                # Handle missing container ID gracefully
                container = await docker_call(bot.docker_client.containers.get, vps.get("container_id", "")) if vps.get("container_id") else None
            ...
            vps_info = (
                f"Owner `{username}`  •  Service `{status.upper()}`  •  Docker `{str(container_status).upper()}`\n"
                f"`{vps.get('memory', 'Unknown')} GB RAM`  •  `{vps.get('cpu', 'Unknown')} vCPU`  •  "
                f"`{vps.get('disk', 'Unknown')} GB storage`\n"
                f"System `{vps.get('os_image', DEFAULT_OS_IMAGE)}`  •  Restarts `{vps.get('restart_count', 0)}`"
            )

            embed.add_field(
                name=f"{EMOJI_START if status == 'Running' else EMOJI_STOP}  {vps.get('vps_id', 'Unknown')}",
                value=vps_info,
                inline=False
            )
            valid_vps_count += 1
...
@app_commands.describe(
    token="Access token for the VPS"  # noqa: S106 - command argument description
)
async def connect_vps(ctx, token: str):
    ...
    try:
        try:
            ...
            logger.warning("Could not open VPS %s for connection: %s", vps['vps_id'], exc)
            await safe_ctx_send(ctx, f"{SYMBOL['error']} VPS instance is not available.", ephemeral=True)
            return
        ...
        embed = build_ssh_access_embed(vps['vps_id'], ssh_session_line)
...
async def admin_stats(ctx):
    ...
    try:
        # Get Docker stats
        containers = await docker_call(bot.docker_client.containers.list, all=True) if bot.docker_client else []
...
async def migrate_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await safe_status_update(status_msg, f"<a:48084loadingcircle:1536290034930548786> Creating snapshot {backup_id} for migration...")
        ...
        await safe_status_update(status_msg, f"<a:8349verify:1536290099526901850> Snapshot {backup_id} created successfully. Please download this file and import it on the new host: {backup_file}")
...
async def emergency_stop(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            try:
                await docker_call(container.stop, timeout=10)
                ...
                logger.warning("Normal stop failed for %s; forcing kill: %s", vps_id, exc)

            # If normal stop failed, try killing the container
            try:
                await docker_call(container.kill)
                bot.db.update_vps(token, {'status': 'stopped'})
                await ctx.send("<a:8349verify:1536290099526901850> VPS killed forcefully!", ephemeral=True)
            except Exception as e:
                raise RuntimeError(f"Failed to kill container: {e}") from e
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Normal stop before force-remove failed for %s: %s", vps_id, exc)

            # Then try to remove it forcefully
            try:
                container = await docker_call(bot.docker_client.containers.get, vps["container_id"])
                await docker_call(container.remove, force=True)
            except docker.errors.NotFound:
                logger.info("Container for %s was already removed", vps_id)
            except Exception as e:
...
async def unsuspend_vps(ctx, vps_id: str):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Could not notify unsuspended VPS owner: %s", exc)
...
async def edit_vps(ctx, vps_id: str, memory: Optional[int] = None, cpu: Optional[int] = None, disk: Optional[int] = None):
    ...
    try:
        ...
        if memory is not None:
            if memory < VPS_MEMORY_GB or memory > MAX_CUSTOM_MEMORY_GB:
                await ctx.send(
                    f"{SYMBOL['error']} RAM must be between {VPS_MEMORY_GB} GB and "
                    f"{MAX_CUSTOM_MEMORY_GB} GB.",
                    ephemeral=True
                )
                return
            updates['memory'] = memory
        if cpu is not None:
            if cpu < DEFAULT_CPU_CORES or cpu > MAX_CUSTOM_CPU_CORES:
                await ctx.send(
                    f"{SYMBOL['error']} CPU cores must be between {DEFAULT_CPU_CORES} "
                    f"and {MAX_CUSTOM_CPU_CORES}.",
                    ephemeral=True
                )
                return
            updates['cpu'] = cpu
        if disk is not None:
            if disk < DEFAULT_DISK_GB or disk > MAX_CUSTOM_DISK_GB:
                await ctx.send(
                    f"{SYMBOL['error']} Disk must be between {DEFAULT_DISK_GB} GB and "
                    f"{MAX_CUSTOM_DISK_GB} GB.",
                    ephemeral=True
                )
                return
            updates['disk'] = disk

        # Build and verify the replacement before deleting the old container.
        old_container = None
        new_container = None
        old_was_running = False
        try:
            old_container = await docker_call(bot.docker_client.containers.get, vps["container_id"])
            old_was_running = old_container.status == 'running'
            if old_was_running:
                await docker_call(old_container.stop)
            memory_bytes = (memory or vps['memory']) * 1024 * 1024 * 1024
            cpu_quota = int((cpu or vps['cpu']) * 100000)
            image = vps.get('os_image') or DEFAULT_OS_IMAGE
            run_options = {
                'detach': True,
                'privileged': True,
                'hostname': f"titancloud-{vps_id}",
            ...
            if not setup_success:
                raise RuntimeError("Failed to configure the replacement container")
            ...
            try:
                ...
                logger.warning("Updated VPS but could not remove old container %s: %s", vps_id, exc)
        except Exception as e:
            logger.error("Error replacing container for VPS %s: %s", vps_id, e)
            if new_container is not None:
                try:
                    await docker_call(new_container.stop)
                    await docker_call(new_container.remove)
                except Exception as cleanup_exc:
                    logger.warning("Could not clean failed replacement container: %s", cleanup_exc)
            if old_container is not None and old_was_running:
                try:
                    await docker_call(old_container.start)
                except Exception as restore_exc:
                    logger.error("Could not restore previous container for %s: %s", vps_id, restore_exc)
            await safe_ctx_send(
                ctx,
                f"{SYMBOL['error']} Update failed. The previous VPS was preserved.",
                ephemeral=True
            )
            return

        bot.db.update_vps(token, updates)
        await safe_ctx_send(ctx, f"{SYMBOL['success']} VPS `{vps_id}` specifications updated successfully!")
...
class VPSManagementView(ui.View):
    def __init__(self, vps_id, container_id):
        ...
        for item in self.children:
            if isinstance(item, discord.ui.Button):
                action = item.label.lower().replace(' ', '_')
                item.custom_id = f"titancloud:{vps_id}:{action}"
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                bot.db.update_vps(token, {'status': 'running'})
            vps['status'] = 'running'
            embed = build_control_panel_embed(self.vps_id, vps, 'running')
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                bot.db.update_vps(token, {'status': 'stopped'})
            vps['status'] = 'stopped'
            embed = build_control_panel_embed(self.vps_id, vps, 'stopped')
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = build_ssh_access_embed(self.vps_id, ssh_session_line)
                            await owner.send(embed=embed)
                        except (discord.Forbidden, discord.HTTPException) as exc:
                            logger.warning("Could not DM restarted VPS SSH details: %s", exc)
                except Exception as exc:
                    logger.warning("Could not rotate SSH after VPS restart: %s", exc)

            vps['status'] = 'running'
            vps['restart_count'] = vps.get('restart_count', 0) + 1
            embed = build_control_panel_embed(self.vps_id, vps, 'running')
    ...
    async def ssh_access(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Revoke every old tmate session, then immediately DM one replacement."""
        try:
            await interaction.response.defer(ephemeral=True)
        except (discord.NotFound, discord.HTTPException) as exc:
            # SSH rotation and DM delivery must continue even when the
            # component webhook can no longer be acknowledged.
            logger.warning("Fresh SSH interaction could not be deferred: %s", exc)

        token, vps = bot.db.get_vps_by_id(self.vps_id)
        if not vps:
            await safe_interaction_send(interaction, f"{SYMBOL['error']} VPS not found.", ephemeral=True)
            return
        if vps.get('status') == 'suspended':
            await safe_interaction_send(
                interaction,
                f"{EMOJI_RESTART} This VPS is suspended. An admin must unsuspend it before SSH access can be rotated.",
                ephemeral=True
            )
            return
        ...
        embed = build_ssh_access_embed(self.vps_id, ssh_session)

        try:
            await interaction.user.send(embed=embed)
            await safe_interaction_send(
                interaction,
                f"{SYMBOL['success']} New SSH access sent by DM. Every previous session is closed.",
                ephemeral=True
            )
        except discord.Forbidden:
            logger.warning("Fresh SSH DM blocked for user %s", interaction.user.id)
            await safe_interaction_send(
                interaction,
            ...
            logger.error("Fresh SSH DM failed for user %s: %s", interaction.user.id, exc)
            await safe_interaction_send(
                interaction,
                f"{SYMBOL['error']} Discord could not deliver the SSH DM. Try again shortly.",
                ephemeral=True
            )
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        ...
        try:
            ...
            try:
                ...
                await safe_status_update(status_msg, f"{EMOJI_LOADING} Pulling the current OS image `{image}`...")
                await docker_call(bot.docker_client.images.pull, image)
                new_container = await docker_call(
                    bot.docker_client.containers.run,
                    image,
            ...
            embed = premium_embed(
                f"{EMOJI_VERIFY}  REINSTALL COMPLETE  /  {self.vps_id}",
            ...
            embed.add_field(name="MEMORY", value=f"`{vps['memory']} GB`", inline=True)
            embed.add_field(
                name=f"{EMOJI_MANAGE}  NEW PRIVATE SSH COMMAND",
                value=f"```bash\n{ssh_session}\n```",
                inline=False
            )
            ...
            await safe_status_update(status_msg, f"{EMOJI_VERIFY} Current OS reinstalled successfully. Details sent by DM.")
            ...
            if committed:
                try:
                    ...
                    logger.debug("Could not report completed reinstall: %s", notify_exc)
                return
            if new_container is not None and not committed:
                try:
                    ...
                    logger.warning("Could not clean failed reinstall container: %s", cleanup_exc)
            await safe_status_update(
                status_msg,
                f"{SYMBOL['error']} Reinstall failed. Your previous VPS was kept online."
            )
    ...
    async def on_timeout(self):
        ...
        try:
            ...
            logger.debug("Could not disable timed-out reinstall view: %s", exc)
        ...
        try:
            ...
            try:
                ...
                if member is None:
                    member = next(
                        (
                            guild.get_member(new_owner.id)
                            for guild in bot.guilds
                            if guild.get_member(new_owner.id) is not None
                        ),
                        None
                    )
                ...
                await safe_interaction_send(interaction, f"{SYMBOL['error']} Invalid user ID or mention!", ephemeral=True)
                return
            ...
            try:
                embed = premium_embed(
                    title=f"{EMOJI_VERIFY}  OWNERSHIP TRANSFER COMPLETE",
                    description="This private VPS is now assigned to you.",
                    color=SUCCESS_COLOR
                )
                embed.add_field(name="INSTANCE", value=f"`{self.vps_id}`", inline=True)
                embed.add_field(name="PREVIOUS OWNER", value=f"`{old_owner_name}`", inline=True)
                add_instance_spec_fields(embed, vps)
                embed.add_field(
                    name=f"{EMOJI_MANAGE}  GET PRIVATE SSH",
                    value=f"Run `/manage {self.vps_id}` and select **Fresh SSH**. The new command will be sent only to your DMs.",
                    inline=False
                )
                await new_owner.send(embed=embed)
            except (discord.Forbidden, discord.HTTPException) as exc:
                logger.warning("Could not DM transferred VPS owner: %s", exc)
                await safe_interaction_send(interaction, f"{SYMBOL['info']} Could not DM the new owner.", ephemeral=True)
...
async def manage(ctx, vps_id: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            logger.warning("Could not inspect container for %s: %s", vps_id, exc)
            container_status = "Not Found"

        embed = build_control_panel_embed(vps_id, vps, container_status)
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        try:
            ...
            embed.add_field(name="INSTANCE", value=f"`{vps_id}`", inline=True)
            embed.add_field(name="PREVIOUS OWNER", value=f"`{ctx.author.name}`", inline=True)
            add_instance_spec_fields(embed, vps)
            embed.add_field(
                name=f"{EMOJI_MANAGE}  GET PRIVATE SSH",
                value=f"Run `/manage {vps_id}` and select **Fresh SSH**. The new command will be sent only to your DMs.",
                inline=False
            )
            await new_owner.send(embed=embed)
        except (discord.Forbidden, discord.HTTPException) as exc:
            logger.warning("Could not DM transferred VPS owner: %s", exc)
            await safe_ctx_send(ctx, f"{SYMBOL['info']} Could not DM the new owner.", ephemeral=True)
...
if __name__ == "__main__":
    try:
        if not TOKEN or TOKEN == 'replace_with_your_bot_token':  # noqa: S105 - placeholder validation
            raise RuntimeError(
